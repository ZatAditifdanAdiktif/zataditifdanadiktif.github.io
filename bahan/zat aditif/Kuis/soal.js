...
$(document).ready(function(){
    var rootRef = firebase.database().ref().child("soalKuis1/");
    ...
    emptyArray.push(id); // push soalid ke emptyarray
      ...
      $(document).ready(function(){
        var rootRef = firebase.database().ref().child("gambar1/");
        rootRef.on("child_added", snap => {
 ...
 //mengambil data link dari firebase
 $(document).ready(function(){
    var rotRef = firebase.database().ref().child("gambar1/");
...
//fungsi menampilkan soal
        function tampil_soal_rand(){
            firebase.database().ref().child("soalKuis1").orderByChild("soalid").equalTo(randArray[panggil]).once("value", function(snapshot){
                snapshot.forEach(function(childSnapshot){
                    var soalid=childSnapshot.child("soalid").val();
                    var soal=childSnapshot.child("soal").val();
                    var opt1=childSnapshot.child("opt1").val();
...
function dataKontrol(){
    ...
    function(AllRecords1){
        AllRecords1.forEach(
            function(CurrentRecord){
                var kontrol=CurrentRecord.val().kuis1;
